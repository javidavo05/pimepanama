  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      <div className="bg-panel border border-line rounded-xl p-5 space-y-4">
        <h3 className="text-fg-dim text-xs uppercase tracking-widest font-medium">Identificación</h3>
        <div>
          <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Nombre descriptivo</label>
      ...
      {/* IMAP */}
      <div className="bg-panel border border-line rounded-xl p-5 space-y-4">
        <h3 className="text-fg-dim text-xs uppercase tracking-widest font-medium">Servidor de entrada (IMAP)</h3>
        <div className="grid grid-cols-3 gap-3">
          <div className="col-span-2">
            <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Servidor</label>
            <input value={form.host} onChange={(e) => set("host", e.target.value)} required placeholder="imap.gmail.com" className={inputCls} />
          </div>
          <div>
            <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Puerto</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Contraseña</label>
        ...
        <div>
          <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Tipo de credencial</label>
          <div className="flex gap-2">
            {[["PASSWORD", "Contraseña normal"], ["PASSWORD_APP", "Contraseña de aplicación (recomendada)"]].map(([val, lab]) => (
              <button key={val} type="button" onClick={() => set("credType", val)}
                className={`px-3 py-1.5 rounded-lg text-xs border transition-all ${form.credType === val ? "bg-brand/10 border-brand/30 text-brand-fg" : "border-line text-fg-dim hover:text-fg-dim"}`}>
                {lab}
              </button>
            ))}
          </div>
          {form.credType === "PASSWORD_APP" && (
            <p className="text-fg-faint text-xs mt-2">Para Gmail: Configuración → Seguridad → Contraseñas de aplicación</p>
          )}
        </div>
        ...
        <div className="pt-1 border-t border-line">
          <div className="flex items-center gap-3">
            <button
              ...
              onClick={handleTest}
              disabled={testing}
              className="flex items-center gap-2 px-4 py-2 bg-fill border border-line hover:bg-fill-2 disabled:opacity-40 text-fg-mute text-sm rounded-lg transition-all"
            >
              {testing ? (
                <>
                  <span className="w-3.5 h-3.5 border-2 border-line-loud border-t-fg-dim rounded-full animate-spin" />
                  Probando conexión…
                </>
              ) : (
                <>
            ...
            {testResult && (
              <span className={`text-xs flex items-center gap-1.5 ${testResult.ok ? "text-ok" : "text-danger"}`}>
                {testResult.ok ? (
                  <>
      ...
      <div className="bg-panel border border-brand/15 rounded-xl p-5 space-y-3">
        <h3 className="text-fg-dim text-xs uppercase tracking-widest font-medium">Envío saliente (Resend)</h3>
        <p className="text-fg-faint text-sm leading-relaxed">
          Los correos se envían vía <strong className="text-fg-mute">Resend</strong> con tracking de entrega y apertura.
          El correo/usuario de esta cuenta se usa como remitente si el dominio está verificado en Resend; si no, se usa <code className="text-fg-dim">RESEND_FROM</code>.
        </p>
        <p className="text-fg-faint text-xs">
          Configura <code className="text-fg-dim">RESEND_API_KEY</code>, <code className="text-fg-dim">RESEND_FROM</code> y el webhook en Vercel. El username debe ser del dominio verificado (ej. javier@pimepanama.com).
        </p>
        {mode === "edit" && accountId && (
          <button
            type="button"
            ...
            className="px-4 py-2 text-xs border border-brand/25 text-brand-fg rounded-lg hover:bg-brand/10 disabled:opacity-50"
          >
            {testingSmtp ? "Enviando prueba..." : "Probar envío Resend"}
          </button>
        )}
        {smtpTestResult && (
          <p className={`text-xs ${smtpTestResult.startsWith("Error") ? "text-danger" : "text-ok"}`}>
            {smtpTestResult}
          </p>
        )}
      </div>
      ...
      <details className="bg-panel border border-line rounded-xl p-5 space-y-4">
        <summary className="text-fg-faint text-xs uppercase tracking-widest font-medium cursor-pointer">
          SMTP legacy (opcional, ya no usado para enviar)
        </summary>
        <div className="grid grid-cols-3 gap-3">
          <div className="col-span-2">
            <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Servidor SMTP</label>
        ...
        {mode === "edit" && accountId && form.smtpHost && (
          <button
            ...
            onClick={handleTestSmtp}
            disabled={testingSmtp}
            className="px-4 py-2 text-xs border border-line text-fg-dim rounded-lg hover:bg-fill disabled:opacity-50"
          >
            {testingSmtp ? "Enviando..." : "Probar SMTP legacy"}
          </button>
        )}
        {smtpTestResult && form.smtpHost && (
          <p className={`text-xs ${smtpTestResult.includes("enviado") || smtpTestResult.includes("ok") ? "text-ok" : "text-danger"}`}>
            {smtpTestResult}
          </p>
        )}
      </details>
      ...
      {form.username && (
        <div className="bg-panel border border-line rounded-xl p-5 space-y-4">
          <h3 className="text-fg-dim text-xs uppercase tracking-widest font-medium">Firma de correo</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Nombre en firma</label>
              <input value={form.signatureName} onChange={(e) => set("signatureName", e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Cargo / título</label>
            ...
            <div>
              <label className="block text-fg-faint text-xs uppercase tracking-widest mb-1.5">Nombre remitente (From)</label>
      ...
      <div className="flex gap-3">
        ...
        <button type="submit" disabled={saving}
          className="px-6 py-2.5 bg-brand hover:bg-brand-hi disabled:opacity-50 text-on-brand text-sm font-semibold rounded-lg transition-all ml-auto">
