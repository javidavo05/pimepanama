...
export default async function CotizacionDetailPage({
  ...
  const statuses: DocumentStatus[] = ["DRAFT", "SENT", "ACCEPTED", "REJECTED", "CANCELLED"];
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-[#0a0a10] border border-white/[0.06] rounded-2xl p-6 grid grid-cols-2 gap-6">
        ...
        <div className="text-right">
          <p className="text-white/40 text-xs uppercase tracking-widest mb-1">Total estimado</p>
          <p className="text-[#C8A96E] font-mono text-2xl font-semibold">
            {doc.currency} {Number(doc.total ?? 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}
          </p>
          {doc.validUntil && (
            <p className="text-white/40 text-xs mt-1">
              Válida hasta: {new Date(doc.validUntil).toLocaleDateString("es-PA")}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="bg-[#0a0a10] border border-white/[0.06] rounded-xl p-5">
        <p className="text-white/60 text-xs uppercase tracking-widest font-medium mb-3">Cambiar estado</p>
        <div className="flex flex-wrap gap-2">
          {statuses.map((s) => (
            <form key={s} action={async () => { "use server"; await updateDocumentAction(id, { status: s }); }}>
              <button type="submit" className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${doc.status === s ? "bg-[#C8A96E]/10 border-[#C8A96E]/30 text-[#C8A96E]" : "border-white/[0.07] text-white/40 hover:text-white/70 hover:border-white/20"}`}>
                {s}
              </button>
            </form>
          ))}
